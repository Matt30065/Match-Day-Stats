...
function openPowerPlayOffDialog(required=false){const match=currentMatch();if(!match||match.halfTime||match.fullTime)return;ensurePowerPlayFields(match);if(!match.powerPlayPlayers.length)return;const byId=Object.fromEntries(loadPlayers().map(p=>[p.id,p]));$('powerPlayOffPlayer').innerHTML=match.powerPlayPlayers.map(id=>byId[id]).filter(Boolean).map(p=>`<option value="${p.id}">${escapeHtml(playerLabel(p))}</option>`).join('');$('powerPlayOffRequired').textContent=required?'A player must be withdrawn because the goal difference has fallen.':'Select a Power Play player to withdraw.';$('powerPlayOffDialog').dataset.required=required?'1':'0';$('powerPlayOffDialog').showModal();}
function recordPowerPlayOff(playerId){const match=currentMatch();if(!match||match.halfTime||match.fullTime)return;ensurePowerPlayFields(match);if(!playerId||!match.powerPlayPlayers.includes(playerId))return;const limit=powerPlayLimit(match);if(match.powerPlayPlayers.length<=limit)return;match.periodElapsedSeconds=elapsedNow();if(match.periodStartedAt)match.periodStartedAt=Date.now();match.currentOnPitch=match.currentOnPitch.filter(id=>id!==playerId);match.currentSubs.push(playerId);match.powerPlayPlayers=match.powerPlayPlayers.filter(id=>id!==playerId);match.events.push({id:makeId(),type:'power_play_off',period:match.period,minute:matchMinute(),playerId});saveCurrentMatch(match);$('powerPlayOffDialog').close();renderLiveUI();}
